...
export const entry = (options = {}) => ({
  ...
  config(userConfig) {
    const name =
      options.name || (options.entry ? path.parse(options.entry).name : 'main')
    ...
    if (!userConfig.build.rollupOptions.output.assetFileNames)
      userConfig.build.rollupOptions.output.assetFileNames = `${name}-[name]-[hash][extname]`
    if (!userConfig.build.rollupOptions.output.chunkFileNames)
      userConfig.build.rollupOptions.output.chunkFileNames = `${name}-[name]-[hash].js`
    if (!userConfig.build.rollupOptions.output.entryFileNames)
      userConfig.build.rollupOptions.output.entryFileNames =
        name + (options.hashEntry ? '-[hash]' : '') + '.js'
    userConfig.base = './'
